...
Allows you to overwrite the default Graph version number set in `Facebook\Facebook::DEFAULT_GRAPH_VERSION`. Set this as a string as it would appear in the Graph url, e.g. `v6.0`. Defaults to the [latest version of Graph](https://developers.facebook.com/docs/apps/changelog).
...
```php
$response = $fb->sendRequest('GET', '/me', [], '{access-token}', 'eTag', 'v6.0');
```
